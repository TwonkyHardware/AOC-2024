    ...
    for path in paths:
        ...
        if trail_map[r][c] == '9':
            ...
            for step in next_steps:
                ...
                # Part 2: We do need every path.
                # Let's just take this out, then.
                """
...
for row in range(height):
    for col in range(width):
        ...
        if trail_map[row][col] == '0':
            ...
            # Record the trailhead and distinct trails
            #summits = [path[-1] for path in paths]
            distinct_trails = len(paths)
            
            trail_data = {'trailhead': (row,col),
                          'distinct trails': distinct_trails}
...
for trail_data in trails:
    map_score += trail_data['distinct trails']
